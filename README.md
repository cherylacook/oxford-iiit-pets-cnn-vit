...
## Objective
Classify images from the Oxford-IIIT Pets dataset — grouped into four broad categories (long-haired cats, short-haired cats, long-haired dogs, short-haired dogs) — using a convolutional neural network (CNN) and a vision transformer (ViT) built from scratch in PyTorch. Examine how architectures with different inductive biases perform when trained on a relatively small image dataset.
