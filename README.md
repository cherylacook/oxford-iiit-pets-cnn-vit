...
## Objective
Classify images from the Oxford-IIIT Pets dataset — re-grouped from 37 breeds into four categories (long-haired cats, short-haired cats, long-haired dogs, short-haired dogs) — using a convolutional neural network (CNN) and a vision transformer (ViT) built from scratch in PyTorch. Compare how architectures with different inductive biases perform when trained on a relatively small image dataset.
